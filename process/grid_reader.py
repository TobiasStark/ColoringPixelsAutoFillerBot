...
# Largest grid the game ships; used to reject bogus dimensions read from memory.
MAX_GRID_DIM = 1000
...
class GridReader:
    ...
    def _is_plausible_storage(self, base: int, require_live: bool = True) -> bool:
        ...
        try:
            ...
            return False
        if not self._dims_plausible(x_max, y_max):
            return False
    ...
    @staticmethod
    def _dims_plausible(x_max: int, y_max: int) -> bool:
        return 0 < x_max <= MAX_GRID_DIM and 0 < y_max <= MAX_GRID_DIM
    ...
    def read_int_array(self, obj_addr: int, x_max: int, y_max: int):
        ...
        # A 100x100 level has 10k cells and is re-read after every viewport, so
        # parse the buffer in one numpy call instead of per-element slicing.
        return np.frombuffer(data, dtype="<i4", count=max_len)

    def to_logical(self, flat, x_max: int, y_max: int):
        """Convert memory-order (column-major, bottom-to-top per column) to logical top-to-bottom rows."""
        arr = np.asarray(flat, dtype=np.int32).reshape(x_max, y_max)
        # arr[x][j] with j = y_max - 1 - y_top: transpose to rows, then flip.
        return np.flipud(arr.T).tolist()
    ...
    def read_grids(self, base: int = None, book_name: str = "Book1", save_file: str = None):
        ...
        if not self._dims_plausible(x_max, y_max):
            raise RuntimeError(
                f"Implausible grid size {x_max}x{y_max} at 0x{base:08X}; "
                "the CrossLevelStorage address is stale (level changed?)"
            )
