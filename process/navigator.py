...
class Navigator:
    ...
    def _screenshot(self):
        ...
        if not hasattr(self, '_vision'):
            self._vision = Vision(self.window.client_rect())
    ...
    def _detect_open_buttons(self, img=None):
        ...
        debug(f"    Detected {len(buttons)} OPEN button candidates")
    ...
    def dismiss_dialog(self, img=None):
        ...
        info(f"  Confirmation dialog detected; clicking CANCEL at ({cx}, {cy})")
    ...
    def _find_open_button(self, target_index, total_levels):
        ...
        if target_row >= total_rows - 1:
            ...
            info(f"  Level {target_index} -> row {target_row} col {target_col} "
    ...
    def goto_level(self, book_title, level_index, max_attempts=4):
        ...
        for attempt in range(max_attempts):
            ...
            if actual_level == level_index:
                info(f"  Level {level_index} ({level['display_name']}) loaded @ 0x{base:08X}")
                return base
            ...
            if correction:
                ...
                info(f"  Wrong level opened (index {actual_level}); "
                 f"top row offset -> {self._top_row_offset}")
            else:
                info(f"  Wrong level opened (index {actual_level}) in the expected row; retrying")
            self.return_to_menu()
            self.goto_book(book_title)
