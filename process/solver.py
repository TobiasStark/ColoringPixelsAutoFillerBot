...
def densest_cell(remaining, viewport_w, viewport_h, cur_x, cur_y):
    """Return the remaining cell with the most other remaining cells around it.

    Ties are broken towards the current viewport centre (`cur_x`, `cur_y`).
    Neighbours are counted with a summed-area table over the level grid, so the
    cost grows with the level size instead of with the square of the number of
    remaining cells — a 100x100 level would otherwise need a 10000x10000 matrix.
    """
    if not remaining:
        return None
    pts = np.array(sorted(remaining), dtype=np.int64)
    xs, ys = pts[:, 0], pts[:, 1]
    x_off, y_off = int(xs.min()), int(ys.min())
    width = int(xs.max()) - x_off + 1
    height = int(ys.max()) - y_off + 1
    gx, gy = xs - x_off, ys - y_off

    occupancy = np.zeros((height, width), dtype=np.int32)
    occupancy[gy, gx] = 1
    # sat[y + 1, x + 1] = number of remaining cells in [0..y] x [0..x].
    sat = np.zeros((height + 1, width + 1), dtype=np.int32)
    sat[1:, 1:] = occupancy.cumsum(0).cumsum(1)

    # |delta| < viewport for integer offsets.
    rx = max(0, int(np.ceil(viewport_w)) - 1)
    ry = max(0, int(np.ceil(viewport_h)) - 1)
    x_lo = np.clip(gx - rx, 0, width - 1)
    x_hi = np.clip(gx + rx, 0, width - 1)
    y_lo = np.clip(gy - ry, 0, height - 1)
    y_hi = np.clip(gy + ry, 0, height - 1)
    counts = (sat[y_hi + 1, x_hi + 1] - sat[y_lo, x_hi + 1]
              - sat[y_hi + 1, x_lo] + sat[y_lo, x_lo])

    dist_sq = (xs - cur_x) ** 2 + (ys - cur_y) ** 2
    best = int((counts.astype(float) * 100 - dist_sq).argmax())
...
class Solver:
    def __init__(self, mapper: HomographyMapper = None, palette_positions: dict = None,
        ...
        self.reader = MemoryReader(process_name=config.PROCESS_EXE)
    ...
    def _densest_remaining(self, remaining, cal):
        ...
        cur_x = ((x0 + x1) / 2 - ox) / dx
        cur_y = ((y0 + y1) / 2 - oy) / dy
        return densest_cell(remaining, (x1 - x0) / dx, (y1 - y0) / dy, cur_x, cur_y)
    ...
    def solve(self, max_viewports=400):
        ...
        for viewport in range(max_viewports):
                    ...
                    if stuck_count <= 5:
                        ...
                        if new_cal is not None:
                            ...
                            if remaining:
                                ...
                                if pan_dx == 0 and pan_dy == 0:
                                    # Already at densest cluster but can't
                                    # calibrate — pan to a different area.
                                    target = self._farthest_remaining(remaining, cal)
                                    pan_dx, pan_dy, capped = self._pan_to(cal, target, info)
                                print(f"  Calibration failed; panning to "
            ...
            if new_cal is not None:
                ...
                # Vision failed (too few cells or noise).  Trust the
                # pan-adjusted prior and try painting — stall detection
                # will catch it if the prior is wrong.
                print(f"  Vision failed; trusting pan-adjusted prior")
                cal = pan_cal
