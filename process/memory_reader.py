...
class MemoryReader:
    ...
    def iterate_regions(self, writable_only: bool = True):
        ...
        while ctypes.windll.kernel32.VirtualQueryEx(h, ctypes.c_void_p(base), ctypes.byref(mbi), mbi_size):
    ...
    def scan_pattern(self, pattern: bytes, writable_only: bool = True, limit: int = 0,
        ...
        for base, size in self.iterate_regions(writable_only=writable_only):
            ...
            while offset < size:
                ...
                try:
                    chunk = self.pm.read_bytes(base + offset, read_size)
                except Exception:
                    offset += chunk_size
                    continue
                start = 0
...
if __name__ == "__main__":
    # basic test
    import psutil
    pids = [p.pid for p in psutil.process_iter(["name"]) if p.info["name"] == "ColoringPixels.exe"]
    print("pids", pids)
