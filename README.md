...
- **Python 3.10+**
- The game window must be visible and unobscured (any resolution is supported; navigation still relies on a `data/ui_config.json` tuned for your screen)
...
These values are a starting point for **1920x1080 fullscreen**. If you use a different resolution, adjust the pixel coordinates in this file.
...
- If you use a non-standard resolution or window size, tune the margin fractions or `ui_config.json` values.
...
Pure-logic unit tests live in `tests/` and can be run without a game instance:
